...
module.exports = {
    ...
    list : function(words, world_id){
      var list = '';
      var world = '';
      if(world_id) {
        world = `world=${world_id}&`;
      }
      // db.query(`SELECT * FROM world`, function(error, worlds){
      //   if(error){
      //     throw error;
      //   }
      //   var tag = '';
      ...
      //   list = `<select name="world">${tag}</select>`; 
        list += `<ul><li><a href="/all">전체보기</a></i>`;
        var j = 0;
        while(j < words.length) {
          list += `<li><a href="/?${world}id=${words[j].id}">${words[j].word}</a></li>`;
          j++;
        }
        list = list + `</ul>`;
        return list;
    ...
    worldSelect(worlds, title, world_id){
    ...
    worldSelectChange(worlds, title, world_id){
      var tag = `<option value="0">세계선택</option>`;
      var i = 0;
      while(i < worlds.length) {
        var selected = '';
