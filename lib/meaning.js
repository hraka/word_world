...
exports.createMeaning_process = function(request, response) {
    ...
    request.on('end', function(){
      ...
      db.query(`
            INSERT INTO meaning (meaning, created, word_id, author_id, source, relation)
              VALUES(?, NOW(), ?, ?, ?, ?)`,
            [post.meaning, post.wid, post.author, post.source, post.relation],
                ...
                db.query(`SELECT * FROM meaning WHERE id = ?`,[queryData.mid], function(error3, meaning){
                    ...
                    db.query(`SELECT * FROM author`, function(error4, authors){
                        ...
                        var html = template.HTML(title, list, `
                            ...
                            <p>연관어: <input type="text" name="relation" placeholder="relation" value="${meaning[0].relation}"></p>
                            <p>출처: <input type="text" name="source" placeholder="source" value="${meaning[0].source}"></p>
...
exports.updateMeaning_process = function(request, response) {
    ...
    request.on('end', function(){
        ...
        db.query(`UPDATE meaning SET meaning=?, author_id=?, source=?, relation=? WHERE id=?`, 
          [post.meaning, post.author, post.soucre, post.relation, post.mid],
          function(error, result){
            if(error) {
              throw error;
            }
