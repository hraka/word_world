...
exports.changeOnOFF = function(request, response) {
    ...
    request.on('end', function(){
      ...
      var new_onoff_value = 1;
      var now_onoff_value = post.now_onoff_value;
      if(now_onoff_value == 'OFF') { //문자열 타입으로 받는 게 좋을까? bool이 낫지는 않을까?
          new_onoff_value = 1;
      } else {
          new_onoff_value = 0;
      }
      ...
      db.query(`
      UPDATE meaning SET onoff=? WHERE id=?`, [new_onoff_value, post.id],
            function(error, result){
              if(error) {
                throw error;
              }
