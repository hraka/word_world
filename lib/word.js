...
exports.home = function(request, response) {
  ...
  var world_id = queryData.world; 
  var sql = '';
    if(queryData.world === undefined || queryData.world == 0) {
      sql = "SELECT * FROM word ORDER BY word";
    }
    else {
      
      sql = `SELECT * FROM word WHERE world_id = ${queryData.world}`;
    } 
    db.query(sql, function(error, words){
      db.query(`SELECT * FROM world`, function(error2, worlds){
        var title = 'Welcome';
        var description = '당신의 언어를 정의하고 세상을 만드세요';
        var list = template.list(words, world_id);
        var HTML = template.HTML(title, list,
            `<h2>${title}</h2>${description}`,
            `<a href="/create">create</a>`,
            `<form>
              ${template.worldSelectChange(worlds, title, world_id)}
            </form>`
        );
...
exports.all = function(request, response) {
  ...
  var queryData = url.parse(_url, true).query;
  var world_id = queryData.world;    
  db.query(`SELECT * FROM word ORDER BY word`, function(error, words){
        if(error) {
          throw error; 
        } 
        ...
        //     throw error2;
          // }
          db.query(`SELECT * FROM meaning LEFT JOIN word ON meaning.word_id=word.id ORDER BY meaning.id DESC limit 15`,
...
exports.word = function(request, response) {
    ...
    var world_id = queryData.world;
    var sql = '';
    if(queryData.world === undefined || queryData.world == 0) { //존재하는 번호여야 한다...
      sql = "SELECT * FROM word ORDER BY word";
    }
    else {
    ...
    db.query(sql, function(error, words){
        ...
        db.query(`SELECT * FROM word LEFT JOIN world ON word.world_id=world.id
          ...
          db.query(`SELECT meaning.id, meaning, created, word_id, author_id, source, relation, context, name FROM meaning JOIN author ON meaning.author_id=author.id WHERE word_id = ?`,[queryData.id],
