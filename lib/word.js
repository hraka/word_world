...
exports.word = function(request, response) {
    ...
    db.query(sql, function(error, words){
        ...
        db.query(`SELECT * FROM word LEFT JOIN world ON word.world_id=world.id
          ...
          db.query(`SELECT meaning.id, meaning, created, word_id, author_id, source, relation, context, name FROM meaning JOIN author ON meaning.author_id=author.id WHERE word_id = ? ORDER BY meaning.id DESC`,[queryData.id],
