...
exports.home = function(request, response) {
  ...
  db.query(sql, function(error, words){
    db.query(`SELECT * FROM world`, function(error2, worlds){
      db.query(`SELECT * FROM author WHERE id = ?`, [author_id], function(error3, author) {
        db.query(`SELECT * FROM issue WHERE author_id = 1`, function(error4, issues) {
          if(selected_world != 0){
            title = `${worlds[selected_world - 1].name} 세계`;
            description = '세계의 단어입니다.';
          }
          var issues = template.issues(issues);
...
exports.homeIssueChange = function(request, response) {
  var selected_world = worldIsSelected(request, response);
  var title = 'Welcome';
  var description = '당신의 언어를 정의하고 세상을 만드세요';
  var sql = '';
    ...
    db.query(`SELECT * FROM world`, function(error2, worlds){
      db.query(`SELECT * FROM author WHERE id = ?`, [author_id], function(error3, author) {
        db.query(`SELECT * FROM issue WHERE author_id = 1`, function(error4, issues) {
          ...
          var issues = template.issuesChange(issues);
          var list = template.list(words);
          var HTML = template.HTML(title, list,
              `
              <h2>${title}</h2>
...
exports.update_issue_process = function(request, response) {
  var body = '';
  ...
  request.on('end', function(){
    var post = qs.parse(body);
    db.query(`DELETE FROM issue WHERE author_id = ?`, [1],
      function(error, result_D){
        if(error) {
          throw error;
        }
        for(var i = 0; i < 5; i++) {
          ...
          if(post['issue' + i] != '') {
            ...
            db.query(`INSERT INTO issue (author_id, issue_id, created, word_name)
              VALUES(1, 0, NOW(), ?)`, [post['issue' + i]],
              function(error, result){
                if(error) {
                  throw error;
                }
    ...
    // console.log(post['issue0']);
    // console.log(post[`issue`+0]);
